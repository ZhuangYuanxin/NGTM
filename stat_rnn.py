...
def sparsity_stats_all_mcmc(graph_ref_list, graph_pred_list):
    def sparsity(G):
        return (G.number_of_nodes()**2-len(G.edges))/ G.number_of_nodes()**2
...
def mmd_eval(generated_graph_list, original_graph_list, diam = False):
    # print('generated_graph_list',generated_graph_list)
    ...
    if degree2 == 5:
        return 5
    print('degree', mmd_degree, 'clustering', mmd_clustering, 'orbits', mmd_4orbits, "Spec:", mmd_spectral, " diameter:" , mmd_diam)
    return('degree: '+str(mmd_degree) +' clustering: ' + str(mmd_clustering) +' orbits: '+str(mmd_4orbits)+" Spec: "+str( mmd_spectral) + " diameter:" + str(mmd_diam) + " average edge # in test set: "+ str(degree1) + " average edge # in grnrated set: "+ str(degree2))


def mmd_eval_mcmc(generated_graph_list, original_graph_list, diam = False):
    generated_graph_list = [G for G in generated_graph_list if not G.number_of_nodes() == 0]
    for G in generated_graph_list:
        G.remove_edges_from(nx.selfloop_edges(G))
    ...
    mmd_4orbits = -1
    mmd_clustering = clustering_stats(original_graph_list, generated_graph_list)
    mmd_sparsity, degree1, degree2 = sparsity_stats_all_mcmc(original_graph_list, generated_graph_list)
    mmd_spectral = spectral_stats(original_graph_list, generated_graph_list)
    if diam:
        mmd_diam = MMD_diam(original_graph_list, generated_graph_list)
    else:
        mmd_diam = "_"
    # print('degree', mmd_degree, 'clustering', mmd_clustering, 'orbits', mmd_4orbits, "Spec:", mmd_spectral, " diameter:" , mmd_diam, "average edge # in test set:" , degree1, " average edge # in generated set: ", degree2)
    mmd_res = mmd_degree + mmd_clustering + mmd_spectral + mmd_diam    
    return mmd_res


# load a list of graphs   Deg. Clus. Orbit. Spect. Diam
