...
# 修改plot_mapping_matrix函数，使其能够遍历并保存所有子结构的映射矩阵热图

def plot_all_mapping_matrices(M, epoch, graph_save_path):
    num_substructures = M.shape[0]  # 子结构的数量

    for substructure_idx in range(num_substructures):
        mapping_matrix = M[substructure_idx]
        
        plt.figure(figsize=(10, 8))
        plt.imshow(mapping_matrix, cmap='hot', interpolation='nearest', aspect='auto')
        plt.colorbar()
        plt.title(f"Mapping Matrix for Substructure {substructure_idx+1} at Epoch {epoch}")
        plt.xlabel("Reconstructed Graph Nodes")
        plt.ylabel("Substructure Nodes")
        
        # 为每个子结构保存图像
        file_name = f"{graph_save_path}training_mapping_subgraph_{substructure_idx+1}_At_epoch{epoch}.png"
        
        # file_name = graph_save_path + "training_mapping_subgraph_" + str(epoch) + ".png"
        plt.savefig(file_name)
        plt.close()  # 关闭图像，防止重复输出

def plot_test_mapping_matrices(M,  graph_save_path, epoch, jjj, g_size, topics,load):
    num_substructures = M.shape[0]  # 子结构的数量
    ...
    for substructure_idx in range(num_substructures):
        ...
        # 为每个子结构保存图像
        # file_name = f"{graph_save_path}training_mapping_subgraph_{substructure_idx+1}_At_epoch{epoch}.png"
        file_name = graph_save_path +str(epoch)+ "j" + str(jjj)+ "g" + str(g_size) + "TEST_mapping_subgraph_"+str(substructure_idx+1)+ load + ".png"
...
def plot_test_mapping_matrices_number(M,  graph_save_path, epoch, jjj, g_size, topics,load):
    num_substructures = M.shape[0]  # 子结构的数量
    ...
    for substructure_idx in range(num_substructures):
        ...
        plt.imshow(mapping_matrix, cmap='hot', interpolation='nearest', aspect='auto')
        # 获取矩阵的维度
        height, width = mapping_matrix.shape

        # 在每个单元格中添加数值
        for i in range(height):
            for j in range(width):
                text = plt.text(j, i, f'{j}',
        ...
        # 为每个子结构保存图像
        # file_name = f"{graph_save_path}training_mapping_subgraph_{substructure_idx+1}_At_epoch{epoch}.png"
        file_name = graph_save_path +str(epoch)+ "j" + str(jjj)+ "g" + str(g_size) + "TEST_mapping_subgraph_num"+str(substructure_idx+1)+ load + ".png"
...
def plot_subgraph(reconstructed_adj, graph_save_path, epoch, topics):
    n = reconstructed_adj.shape[0]
    cols = 2
    graphs_per_page = cols * cols

    for page in range(math.ceil(n / graphs_per_page)):
        fig, axes = plt.subplots(cols, cols, figsize=(cols * 5, cols * 5))
        
        for i in range(graphs_per_page):
            index = page * graphs_per_page + i
            if index >= n:
                break
            
            sample_graph = reconstructed_adj[index]
            ...
            if not nx.is_empty(G):
                nx.draw(G, ax=ax, with_labels=False, node_size=150, width=3, node_color='black', edge_color='black')
                ax.set_title(f"Graph {index+1}-topic-{topics[index]}")

        # Hide unused subplots
        for j in range(i + 1, graphs_per_page):
            fig.delaxes(axes[j // cols, j % cols])
        
        file_name = f"{graph_save_path}training_generated_subgraph_At_epoch{epoch}_page{page+1}.png"
        plt.savefig(file_name)
        plt.close()
        

# def plot_subgraph(reconstructed_adj, graph_save_path, epoch, topics):
#     # 假设 reconstructed_adj 是您的三维 tensor [n, a, a]
#     # print('reconstructed_adj',reconstructed_adj)
#     n = reconstructed_adj.shape[0]
#     a = reconstructed_adj.shape[1]
#     # reconstructed_adj = torch.sigmoid(reconstructed_adj)
#     # 计算行和列的数量，尝试使行和列的数量大致相等
#     rows = cols = int(math.ceil(math.sqrt(n)))

#     fig, axes = plt.subplots(rows, cols, figsize=(cols * 5, rows * 5))

#     # 遍历 reconstructed_adj 的每个子图
#     for i in range(n):
#         sample_graph = reconstructed_adj[i]
#         sample_graph[sample_graph > 0.5] = 1
#         sample_graph[sample_graph < 0.5] = 0
#         # sample_graph[sample_graph > 0] = 1
...
#         if not nx.is_empty(G):
#             nx.draw(G, ax=ax, with_labels=False, node_size=150, width=3, node_color='black', edge_color='black')
#             ax.set_title(f"Graph {i+1}-topic-{topics[i]}")

#     # 隐藏空白的子图
#     for j in range(i+1, rows*cols):
#         fig.delaxes(axes[j // cols, j % cols])
#     file_name = graph_save_path + "training_generated_subgraph_At_epoch" + str(epoch) + ".png"
#     plt.savefig(file_name)
#     plt.close()

def plot_subgraph_test(reconstructed_adj, graph_save_path, epoch, jjj, g_size, topics, load):
    n = reconstructed_adj.shape[0]
    cols = 2
    graphs_per_page = cols * cols
    ...
    for page in range(math.ceil(n / graphs_per_page)):
        ...
        for i in range(graphs_per_page):
            ...
            if not nx.is_empty(G):
                nx.draw(G, ax=ax, with_labels=False, node_size=150, width=3, node_color='black', edge_color='black')
                ax.set_title(f"Substructure {index+1}-topic-{topics[index]}", fontsize=20)

        # Hide unused subplots
        for j in range(i + 1, graphs_per_page):
            fig.delaxes(axes[j // cols, j % cols])
        
        file_name = f"{graph_save_path}{epoch}j{jjj}g{g_size}TEST_generated_subgraph_{load}_page{page+1}.png"
        plt.savefig(file_name)
        plt.close()
...
#         if not nx.is_empty(G):
#             # nx.draw(G, ax=ax, with_labels=True, node_color='black', edge_color='gray')
#             nx.draw(G, ax=ax, with_labels=False, node_size=150, width=3, node_color='black', edge_color='black')
#             ax.set_title(f"Substructure {i+1}-topic-{topics[i]}", fontsize=20)
...
#     # 保存图像
#     file_name = graph_save_path +str(epoch)+ "j" + str(jjj)+ "g" + str(g_size) + "TEST_generated_subgraph_"+ load + ".png"
#     plt.savefig(file_name)
#     plt.close()



def plot_subgraph_topic(reconstructed_adj, graph_save_path, epoch):
    num_topics = reconstructed_adj.shape[0]
    num_samples_per_topic = reconstructed_adj.shape[1]
    cols = 2
    graphs_per_page = cols * cols

    for topic_idx in range(num_topics):
        for page in range(math.ceil(num_samples_per_topic / graphs_per_page)):
            fig, axes = plt.subplots(cols, cols, figsize=(cols * 5, cols * 5))
            
            for i in range(graphs_per_page):
                sample_idx = page * graphs_per_page + i
                if sample_idx >= num_samples_per_topic:
                    break
                
                sample_graph = reconstructed_adj[topic_idx, sample_idx]
                sample_graph = sample_graph.detach().cpu().numpy()  # 转换为 NumPy 数组
                sample_graph[sample_graph > 0.5] = 1
                sample_graph[sample_graph < 0.5] = 0
                ...
                if not nx.is_empty(G):
                    nx.draw(G, ax=ax, with_labels=False, node_size=150, width=3, node_color='black', edge_color='black')
                    ax.set_title(f"Topic {topic_idx}, Sample {sample_idx+1}, Epoch {epoch}")

            # Hide unused subplots
            for j in range(i + 1, graphs_per_page):
                fig.delaxes(axes[j // cols, j % cols])
            
            file_name = f"{graph_save_path}topic_subgraph_At_epoch{epoch}_topic{topic_idx}_page{page+1}.png"
            plt.savefig(file_name)
            plt.close()
...
def plotG(G, type, graph_name = "Generated Graph", file_name=None, plot_it=False):
    ...
    nx.draw(G, with_labels=False, node_size=80, width=3, node_color='black', edge_color='black')

    f.savefig(type+ "_graph.png" if file_name == None else file_name)
    # if plot_it: plt.show()
    plt.close()
    
def plotG_number(G, type, graph_name="Generated Graph", file_name=None, plot_it=False):
    plt.close(graph_name)
    pos = nx.spring_layout(G)
    plt.figure(graph_name)

    nx.draw(G, pos, with_labels=False, node_size=80, width=3, node_color='black', edge_color='black')
    
    # 添加标签
    for node, (x, y) in pos.items():
        plt.text(x, y, s=node, horizontalalignment='center', verticalalignment='center_baseline', color='red', fontsize=15)
    
    plt.savefig(type + "_graph_number.png" if file_name is None else file_name)
    plt.close()
    # if plot_it:
    #     plt.show()

def plotG_r(G, type, graph_name = "Generated Graph", file_name=None, plot_it=False):
    plt.close(graph_name)

    # pos = nx.spring_layout(G, iterations=1000)
    f = plt.figure(graph_name)
    # nx.draw(G, node_size=20, width=1, edge_color="black")
    nx.draw(G, node_size=80, width=4, node_color='black', edge_color='gray')
...
def plot_both( origianl_graphs,generated_graphs, origi_i, gen_i, plot_name="original_generated"):
    ...
    for i in range(len( origianl_graphs)):
        ...
        nx.draw(generated, pos, node_size=150, width=4)
        ...
        plt.subplot(220+counter)
        nx.draw(orig, pos, node_size=150, width=4)
        if i != len(origianl_graphs) - 1:
...
#         cdict = {}
#         for i, key in enumerate(['red', 'green', 'black']):
#             this_cdict = {}
